...
void rayDirections(uint2 tid [[thread_position_in_grid]],
    ...
    tex.write(float4(rays[tid.x + tid.y * tex.get_width()].ray.direction * 0.5 + 0.5, 1), tid);
...
void getNormals(uint2 tid [[thread_position_in_grid]],
    ...
    Intersection intersection = trace(rays[id].ray, scene, types, objectCount);
...
void getDirectLighting(uint2 tid [[thread_position_in_grid]],
    ...
    tex.write(float4(getReflectance(matT, materials) * abs(dot(intersection.n, ray.ray.direction)) + getEmission(matT, materials), 1), tid);
}
